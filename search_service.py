...
class SearchService:
    ...
    def search(self, produto: str, veiculo: str, detalhe: str = "") -> Dict[str, Any]:
        ...
        produtos_info = {p["codigo"]: p for p in self.repo.get_products_by_codes(final_codes)}
        estoque_preco = self.fb.fetch_stock_price_by_codes(final_codes)

        items: List[Dict[str, Any]] = []
        for code in final_codes:
            desc = produtos_info.get(code, {}).get("descricao", "(sem descrição no cache)")
            sp = estoque_preco.get(code, {"estoque": None, "preco": None})
            items.append({"codigo": code, "descricao": desc, "estoque": sp.get("estoque"), "preco": sp.get("preco")})
