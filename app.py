...
import threading, webbrowser, time, configparser
...
async def search(request: Request,
    ...
    sync_service.auto_sync()
    try:
        results = search_service.search(produto=produto, veiculo=veiculo, detalhe=detalhe)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
    template = env.get_template("results.html")
    return template.render(produto=produto, veiculo=veiculo, detalhe=detalhe, results=results)

@app.get("/api/search")
async def api_search(produto: str = "", veiculo: str = "", detalhe: str = ""):
    sync_service.auto_sync()
    return JSONResponse(search_service.search(produto=produto, veiculo=veiculo, detalhe=detalhe))
...
async def admin_add_aplicacao(request: Request,
                              ...
                              ano_fim: int = Form(...),
                              motor: str = Form("")):
    repo.upsert_vehicle(marca.strip(), modelo.strip(), ano_inicio, ano_fim, motor.strip())
    v = repo.find_vehicle(marca, modelo, ano_inicio, ano_fim, motor)
    if not v: raise HTTPException(status_code=400, detail="Veículo não encontrado após inserir.")
